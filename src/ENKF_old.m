...
%     Czz = H*Cyy*H';
    ...
    % Covariance inflation (not used)
    lambda1 = 0;
    ms1 = size(ZMEAS,1);
    inflation_matrix = eye(ms1);
    inflation_matrix(1:ms1+1:end) = diag(Czz);
    Czz = Czz + lambda1*inflation_matrix;
%     Cyy=((Y_ENS-ymean)*(Y_ENS-ymean)')/(Nreps-1);
...
%     figure
%     imagesc(kalm), title('K case'),colorbar
%     figure
...
%     prior_mean_discharge = state_model_dumb(mean(prior_runoff_ensemble,3), H);
    post_runoff_ensemble = unfliparoom(exp(Y_UPDATE), n, s);
    post_mean_discharge = state_model_dumb(mean(post_runoff_ensemble,3), H);
